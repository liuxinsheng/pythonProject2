...
def otcbook(coin):
    ...
    new_book=[]
    for ele in res:
        if float(ele['quoteMaxAmountPerOrder']) > quoteMaxAmountPerOrder and ele['nickName'] !=name1 or ele['nickName'] == name:
            new_book.append(ele)
    # print(new_book)
    return new_book
...
def rob(coin):
    ...
    difference_percentage=round((float(ask[index[0]]['price'])-float(ask[index[1]]['price']))/float(ask[index[1]]['price'])*100,2)
    ...
    elif float(ask[index[0] + 1]['price']) / float(ask[index[0]]['price']) >= 1.000190 and ask[index[0]]['nickName'] == name:
